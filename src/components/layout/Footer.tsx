...
export default function Footer() {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        showNotification("Welcome to the Orbit! Check your email.", "success");
        setEmail('');
      }
    } catch (err) {
      console.error(err);
    ...
    <footer className="border-t border-white/5 py-32 mt-32 bg-background relative overflow-hidden">
      ...
      <div className="max-w-[1700px] mx-auto px-6 md:px-12 w-full grid grid-cols-1 md:grid-cols-12 gap-20">
        <div className="md:col-span-4 flex flex-col gap-8">
          <Link href="/" className="font-black text-4xl tracking-tighter uppercase text-foreground italic group">
            FIT & <span className="text-accent group-hover:text-blue-400 transition-colors">FAB</span>
          </Link>
          <p className="text-foreground/40 font-medium text-base leading-relaxed max-w-sm">
            Clothes that fit you well and make you look good.
          </p>
          <div className="flex gap-4">
            <a href="mailto:hello@fitandfab.com" className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center hover:bg-accent hover:text-white transition-all border border-white/5 hover:border-transparent">
        ...
        <div className="md:col-span-4 flex flex-col gap-8">
           ...
           <form onSubmit={handleSubscribe} className="relative">
             ...
             <button 
               ...
               className="absolute right-2 top-2 bottom-2 px-8 bg-foreground text-background font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-accent hover:text-white transition-all disabled:opacity-50"
             >
               {isSubmitting ? "..." : "Go"}
             </button>
           </form>
      ...
      <div className="max-w-[1700px] mx-auto px-6 md:px-12 w-full mt-32 pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
        <p className="text-[10px] font-bold text-foreground/30 uppercase tracking-[0.3em]">
          &copy; {new Date().getFullYear()} FIT AND FAB ORIGINALS. All rights reserved.
        </p>
